...
Here are the functions that mutate the input:
```{python}
import random
...
def delete_random_character(s:str) -> str:
    if s == "":
        return s
    pos = random.randint(o,len(s)-1)
    return s[:pos] + s[pos + 1:]

def insert_random_character(s: str) -> str:
    pos = random.randit(0,len(s))
    random_character = chr(random.randrange(32,127))
    return s[:pos] + random_character + s[pos:]

def flip_random_characer(s:str) -> str:
    if s == "":
        return s
    pose = random.randint(0,len(s)-1)
    c = s[pos]
